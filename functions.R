load_data <- function(ids) {
  ...
  for (i in 1:length(ids)) {
    ...
    new_cleaned <- clean_events(new_events)
    tmp_events <- bind_rows(tmp_events, new_events)
    tmp_cleaned <- bind_rows(tmp_cleaned, new_cleaned)
    out <- list()
    out$events <- tmp_events
    out$cleaned <- tmp_cleaned
  }
  return(out)
}
...
clean_events <- function(events, min_date, max_date) {
  ...
  events$responseText <- as.list(events$responseText)
  events$version_num <-
...
#====================pull_pages=================================================
pull_pages <- function(cleaned) {
  out <- cleaned %>% distinct(pageNumber) %>% nrow()
  return(as.numeric(out))
}

#====================pull_components=================================================
pull_components <- function(cleaned) {
  out <- cleaned %>% distinct(componentName)
  return(out)
}